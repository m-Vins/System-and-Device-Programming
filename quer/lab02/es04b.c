...
void *visit(char *path, FILE *fp)
{
    ...
    while ((dirp = readdir(dp)) != NULL) // list all the entries
    {
        ...
        {                                                                           // it is a subdirectory --> recursion
            ...
            visit(array_next_paths[n_next_paths], fp);
...
void *thread_function(void *path_arg)
{
    char *path = (char *)path_arg;
    FILE *fp;
    char fileout[MAXLEN];

    sprintf(fileout, "%ld.txt", pthread_self());
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < N; i++)
    {
        pthread_create(&tids[i], NULL, thread_function, (void *)argv[i + 1]);
    }
    ...
    for (int t = 0; t < N; t++)
    {
        ...
        sprintf(filename, "%ld.txt", tids[t]);
        ...
        while ((nr = fread(buffer, 1, BUFFLEN, fp)) > 0)
        {
            nw = fwrite(buffer, 1, nr, stdout);
            if (nr != nw)
            {
